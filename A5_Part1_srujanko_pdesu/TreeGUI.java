	...
	public TreeGUI() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (is_new_tree) {
					...
					try {AbsTree t11;
						...
						t11 = tree.find(Integer.parseInt(s));
						if(t11!=null) {
							JOptionPane.showMessageDialog(null, ""+s+" is already present in the tree");
							return;}
						else {
		...
		deleteButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (b) 
					 tm.set_state(tree);
				else JOptionPane.showMessageDialog(null, "Unable to delete. Not in tree or tree has only one value " + n);
			...
			public void actionPerformed(ActionEvent e) {
				...
				{	
					if(tree.get_count()>1)
					{
						...
						tm.clear();
						outputPanel.clearPanel();
					//JOptionPane.showMessageDialog(null, "Unable to undo because tree has only one value " + tree.value);
						}
				}
				else {
					t=tm.get_state();
...
class TreeMemento {
	...
	public void set_state(AbsTree t) {
		AbsTree t1;
		if(state.isEmpty()) {state.push(t.clone());}
		if (!state.isEmpty()) {
		t1=state.pop();
		if(t1==t) {System.out.println("hi");}
		if(t1!=t) {
			state.push(t1.clone());
			state.push(t.clone());}
...
// insert() calls protected abstract add_node()
	...
	protected void case1b(AbsTree t, AbsTree root) {
