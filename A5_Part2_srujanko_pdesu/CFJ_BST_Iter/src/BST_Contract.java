...
class BST_Iterator {
	Tree stack_peek;
	Tree x;
	int initial_node_min;
    @Requires({"(root.left == null || (root.value > root.left.max() && root.left.ordered()))" ,
    					   "(root.right == null || (root.value < root.right.min() && root.right.ordered()))"})
    @Ensures("stack_peek.value == root.min()")
...
//			System.out.print(p);
    ...
    @Ensures("stack.isEmpty() || node ==null || initial_node_min == stack.peek().value")
	private void stack_left_spine(Tree node) {
		if (node == null) 
			return;
		Tree x = node;
		initial_node_min = x.min();
		stack.push(node);
